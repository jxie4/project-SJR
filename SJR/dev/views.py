...
from .models import COUNTRIES_DICT, VARIABLES_DICT, SOCIAL_INDICATORS_DICT
...
def pic(request, country, variable):
    ...
    df_dropna = df[np.isfinite(df[indicator])]
    plt.ylim((min(df_dropna[indicator])-1, max(df_dropna[indicator])+1))
...
def correlate(request, variable = None):
    ...
    indicator1 = 'Polity Score'
    indicator2 = SOCIAL_INDICATORS_DICT.get(variable)
